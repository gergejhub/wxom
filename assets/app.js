...
TILE_TOOLTIP.met_min_best = {title:"MINIMA CRIT (METAR)", om:"Approach minima (config/airport_minima.json)", why:"Airport-specific approach minima check. Triggers when METAR ceiling or effective visibility (min of VIS and RVRmin) is below the configured BEST minima.", triggers:["METAR: ceiling < BEST.cig_ft (if provided)", "or METAR: min(VIS, RVRmin) < BEST.vis_m (if provided)", "Thresholds from config/airport_minima.json → best"]};
TILE_TOOLTIP.met_min_onlybest = {title:"MINIMA LIMIT (METAR)", om:"Approach minima (config/airport_minima.json)", why:"Triggers when METAR is below the configured 2nd-best (ALT) minima but still NOT below BEST. Operationally: only the BEST approach remains within minima.", triggers:["NOT below BEST minima", "BUT below ALT minima (ceiling or min(VIS, RVRmin))", "Thresholds from config/airport_minima.json → alt"]};
